      ...
      for (var j = numPv-1; j < Math.min(numPv, json.length); j++) {
        ...
        $.get(topUrl, function(topData, topStatus) {
          if (topStatus == 'success' && topData.status == 'ok'){
            ...
            if (depth >= 0) {
              ...
              $.get(url1, function(data1, status1) {
                if (status1 == 'success' && data1.status == 'ok'){
                  ...
                  var leafNodeInfo = `Score of <b>${json[numPv-1].san}</b> after ${1 + depth} plies: <b>${mateScores(score)}</b>`;
            ...
            document.getElementById(idStr).innerHTML = `Score of <b>${json[numPv-1].san}</b> after ${1} plies: <b>${mateScores(json[numPv-1].score)}</b>`;
          }
        });
      }
    }
  ...
  var probe_book = function() {
    ...
    $.get(pvUrlGet, function(data, status) {
      if (status !== 'success'){
        ...
        var line = `Score: ${mateScores(data.score)} Depth: ${data.depth}<br>${pv}`;
